...
//      std::cout<<i<<std::endl<<joint[i].S<<std::endl;
  ...
  for (int i = 1; i < 4; i++) {
    joint[i].vel_joint =
        joint[i - 1].vel_joint + skew(joint[i - 1].avel_joint) * (joint[i - 1].jointRot_W * joint[i].jointPos_B);
...
void initialize_body() {
  ...
  const double TrunkMass = 4.713, ImuMass = 0.001, TrunkImuMass = 4.714, HipMass = 0.696, ThighMass = 1.013, CalfMass = 0.166, FootMass = 0.06, CalfFootMass =
      0.166 + 0.06;
...
void get_acc(const Eigen::VectorXd &gv) {
  ...
  for (int i = 1; i < 4; i++) {
    joint[i].acc_joint =
        joint[i - 1].acc_joint + skew(joint[i - 1].aacc_joint) * joint[i - 1].jointRot_W * joint[i].jointPos_B
...
void forward_body() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    body[i].bias.tail(3) =
        skew(joint[i].avel_joint) * (body[i].I_W - body[i].mass * skew(rac) * skew(rac)) * joint[i].avel_joint;
  }
...
void composite() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    comp[3 - i].c_I_W =
        temp_I + body[3 - i].I_W - temp_mass * skew(r1) * skew(r1) - body[3 - i].mass * skew(r2) * skew(r2);
